...
<div class="d-flex justify-content-between align-items-center mb-3 no-print">
    <h5 class="mb-0"><i class="bi bi-boxes me-1 text-primary"></i> Current Stock Report</h5>
</div>

<div class="card mb-3 no-print">
    <div class="card-header py-2">Filter</div>
    <div class="card-body py-2">
        <form method="GET" action="{{ route('reports.stock.current') }}">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2">
                    <label class="form-label">Show Zero Stock</label>
                    <div class="form-check mt-1">
                        <input class="form-check-input" type="checkbox" name="show_zero" value="1"
                               id="chkZero" {{ $showZero ? 'checked' : '' }}>
                        <label class="form-check-label" for="chkZero">Include Zero Stock</label>
                    </div>
                </div>
                <div class="col-md-4 d-flex gap-2">
                    ...
                    <a href="{{ route('reports.stock.current.export') }}?{{ http_build_query(request()->query()) }}"
...
<div class="d-none d-print-block text-center mb-3">
    <h4 class="mb-0">Current Stock Report</h4>
    <div style="font-size:0.85rem;">As on {{ now()->format('d-M-Y') }}</div>
</div>

<div class="card">
    <div class="card-header py-2 d-flex justify-content-between no-print">
        <span><i class="bi bi-table me-1"></i>Current Stock</span>
        <span class="text-muted" style="font-size:0.80rem;">As on {{ now()->format('d-M-Y') }}</span>
    </div>
    <div class="card-body p-0">
        ...
        <table class="table table-bordered rpt-table mb-0">
            ...
            @foreach($catGroups as $catName => $items)
                ...
                @php $cl = $item->ob + $item->rcpts - $item->issues; @endphp
                <tr class="{{ $cl == 0 ? 'stock-zero' : '' }}">
                    <td>{{ $item->mat_code }}</td>
                    <td>{{ $item->mat_name }}</td>
                    <td>{{ $catName }}</td>
                    <td>{{ $item->uom_name }}</td>
                    <td class="text-end">{{ indianFmt($item->ob, 3) }}</td>
                    <td class="text-end">{{ indianFmt($item->rcpts, 3) }}</td>
                    <td class="text-end">{{ indianFmt($item->issues, 3) }}</td>
                    <td class="text-end fw-semibold">{{ indianFmt($cl, 3) }}</td>
                </tr>
                @php
