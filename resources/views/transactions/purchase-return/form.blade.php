@extends('layouts.app')
@section('title', $hdr ? 'Edit Purchase Return #'.$hdr->inv_no : 'New Purchase Return')
...
<style>
    ...
    .total-card .net-row { background: #e67e22; color: #fff; border-radius: 4px; padding: 4px 8px; }
...
@php
    $isEdit     = !is_null($hdr);
    $formAction = $isEdit
        ? route('transactions.purchase-return.update', [$hdr->br_code, $hdr->inv_no])
        : route('transactions.purchase-return.store');
    $method     = $isEdit ? 'PUT' : 'POST';
@endphp

<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0">
        <i class="bi bi-cart-dash me-1 text-warning"></i>
        {{ $isEdit ? 'Edit Purchase Return #'.$hdr->inv_no : 'New Purchase Return' }}
    </h5>
    <a href="{{ route('transactions.purchase-return.index') }}" class="btn btn-secondary btn-sm">
...
<form method="POST" action="{{ $formAction }}" id="prtnForm">
    @csrf
    @if($isEdit) @method('PUT') @endif

    {{-- ── HEADER ──────────────────────────────────────────────────────── --}}
    <div class="card mb-3">
        <div class="card-header py-2 text-white" style="background:#e67e22;">
            <i class="bi bi-cart-dash me-1"></i> Purchase Return — Header
        </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-1">
                    ...
                    <input type="text" class="form-control" value="{{ $nextInvNo }}" readonly
                           style="background:#e9ecef; font-weight:700; color:#e67e22;">
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <select name="rtn_type" id="rtn_type" class="form-select" required>
                        <option value="">-- Select --</option>
                        @foreach(['COVERING','PLATING'] as $rt)
                        <option value="{{ $rt }}"
                            {{ old('rtn_type', $hdr->rtn_type ?? '') === $rt ? 'selected' : '' }}>
                            {{ $rt }}
                        </option>
                        @endforeach
                    </select>
                ...
                {{-- Reference Purchase Invoice (Optional) --}}
                <div class="col-12">
                    <div class="ref-invoice-box px-3 py-2">
                        <div class="row g-2 align-items-end">
                            <div class="col-auto">
                                <small class="fw-bold text-warning-emphasis">
                                    <i class="bi bi-link-45deg me-1"></i>Reference Purchase Invoice (Optional)
                                </small>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-auto">
                                <small class="text-muted">Select supplier first, then choose reference invoice</small>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    {{-- ── DETAIL ROWS ──────────────────────────────────────────────────── --}}
    <div class="card mb-3">
        <div class="card-header py-2 d-flex justify-content-between align-items-center" style="background:#e67e22; color:#fff;">
            <span><i class="bi bi-table me-1"></i> Item Details</span>
            <button type="button" id="addRowBtn" class="btn btn-sm btn-light">
    ...
    <div class="row align-items-start">
        <div class="col-md-6">
            ...
            <div class="mt-3 d-flex gap-2">
                ...
                <a href="{{ route('transactions.purchase-return.index') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </div>
...
productsMap[{{ json_encode($p->mat_code) }}] = {
    ...
    purch_rate: {{ (float)($p->purch_rate ?? 0) }},
};
@endforeach
...
const productOptions = (function () {
    ...
    @foreach($products as $p)
    html += `<option value="{{ $p->mat_code }}" data-uom="{{ $p->uom }}" data-uomname="{{ addslashes($p->uomUnit->uom_name ?? '') }}" data-rate="{{ $p->purch_rate ?? 0 }}">{{ addslashes($p->mat_code.' - '.$p->mat_name) }}</option>`;
    @endforeach
    return html;
})();
    ...
    <tr id="drow-${idx}" class="detail-row">
        ...
        <td>
            <input type="number" name="items[${idx}][amount]" class="form-control form-control-sm amt-inp"
                   value="${d.amount||''}" readonly style="width:100px; background:#e9ecef;">
        </td>
        <td>
...
$('#detailTbody').on('change', '.prod-sel', function () {
    ...
    row.find('.rate-inp').val(p.purch_rate || '');
    calcRowValue(row);
});
...
function calcTotals() {
    let gross = 0;
    $('#detailTbody .amt-inp').each(function () { gross += parseFloat($(this).val()) || 0; });
...
// Supplier select2
$('#party_code').select2({ theme: 'bootstrap-5', placeholder: 'Search supplier...', width: '100%' });

// When supplier changes, load their purchase invoices
$('#party_code').on('change', function () {
    ...
    $.getJSON('{{ route("transactions.purchase-return.supplierPurchases", "") }}/' + partyCode + '?br_code=' + brCode,
        function (data) {
            data.forEach(function (inv) {
                const date = new Date(inv.inv_date).toLocaleDateString('en-IN', {day:'2-digit', month:'short', year:'numeric'});
                $('#refInvSelect').append(
                    `<option value="${inv.inv_no}">#${inv.inv_no} — ${date} — ₹${parseFloat(inv.nett).toFixed(2)}</option>`
                );
            });
        }
    );
...
$('#loadRefBtn').on('click', function () {
    ...
    const brCode = $('#br_code').val();
    if (!invNo) { alert('Select a reference purchase invoice first.'); return; }

    $.getJSON('{{ route("transactions.purchase-return.purchaseItems", ["__BR__", "__INV__"]) }}'
        .replace('__BR__', brCode).replace('__INV__', invNo),
        function (data) {
        ...
        if (oldItems && oldItems.length > 0) {
            oldItems.forEach(function (item) {
                ...
                addRow({ mat_code: item.mat_code, qty: item.qty, uom: item.uom || pData.uom_code || '',
                         uom_name: pData.uom_name || '', rate: item.rate, amount: item.amount, narration: item.narration || '' });
            });
        } else { addRow(); }
